...
const Cart = () =>{
    ...
    for (let i = 0; i < 12; i++){
            ...
            arrProduct.push(allProducts[i-1])
        }
    }
    ...
    return(
        <div className="Cart">
            { arrProduct.map( arrProduct=>
                <div key={arrProduct.name} className="d-flex justify-content-between align-items-center">
                    <img src={arrProduct.img}/>
                    <div className="col-6">
                        <div>Название: {arrProduct.name}</div>
                        <div>Описание: {arrProduct.description}</div>
                    </div>
                    <div>Цена {arrProduct.price} ₽</div>
                    <button onClick={e => deleteProduct(arrProduct.id, e)}>Удалить</button>
                </div>
             )}
        </div>
    )
